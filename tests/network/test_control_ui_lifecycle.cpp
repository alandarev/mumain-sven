...
{
using nlohmann::json;
using SEASON3B::UiLifecycleFixture;
          ...
          "[network][control-ui]")
{
    ...
    const auto unavailable = App::Control::UiObservabilityObject();
    {
        ...
        fixture.registry.RemoveUIObj(SEASON3B::INTERFACE_SIEGEWARFARE);
        const auto retired = List();
        CHECK(retired["observability"]["siegewarfare_child_active"].is_null());
        RefuseFresh("active or unknown prerequisite: siegewarfare_child_active");
        fixture.registry.AddUIObj(SEASON3B::INTERFACE_SIEGEWARFARE, &fixture.siege);
        LoadingWorld = 30;
...
TEST_CASE("Registered chat producer distinguishes owned whisper ordinary and unrelated input [network][control-ui]")
{
    ...
    for (const bool whisper : {false, true})
    {
        ...
        fixture.registry.RemoveUIObj(SEASON3B::INTERFACE_CHATINPUTBOX);
        CHECK(List()["observability"]["input_focus_owner"].is_null());
        RefuseStale(focused);
        RefuseFresh("active or unknown prerequisite: input_focused");
        fixture.registry.AddUIObj(SEASON3B::INTERFACE_CHATINPUTBOX, &fixture.chat);
...
TEST_CASE("Registered Friends producer retains delayed message blocker after child retirement [network][control-ui]")
{
    ...
    auto destroy = [](SEASON3B::CNewUIFriendWindow* friends)
    {
        delete friends;
        Core::Time::FrameTimerScheduler::Instance().Kill(CHATCONNECT_TIMER);
    };
    std::unique_ptr<SEASON3B::CNewUIFriendWindow, decltype(destroy)> friends(new SEASON3B::CNewUIFriendWindow, destroy);
          ...
          "[network][control-ui]")
{
    ...
    for (const char* drift : {"geometry", "index", "peer_index", "context", "held", "edge", "queued", "unavailable"})
    {
        ...
        if (kind == "unavailable")
            fixture.registry.RemoveUIObj(SEASON3B::INTERFACE_HOTKEY);
        if (kind == "context")
        ...
        if (kind == "unavailable")
            fixture.registry.AddUIObj(SEASON3B::INTERFACE_HOTKEY, &fixture.hotkey);
